...
export default function LcrSearchAndPrint({ title, type }) {
  ...
  const handleSelectClient = (client) => {
    ...
    if (client.data) {
      ...
      setLcrSex(client.data.sex || '')
      setLcrBirthDate(client.data.dateOfBirth || '')
      setLcrPlaceBirth(client.data.placeOfBirth || '')
      setLcrMotherName(client.data.motherName || '')
      setLcrFatherName(client.data.fatherName || '')
      setLcrCitizenshipMother(client.data.motherCitizenship || '')
      setLcrCitizenshipFather(client.data.fatherCitizenship || '')
    }
  }
  ...
  const renderSupplementalAffidavit = () => {
    return (
      <div className="bg-white p-12 shadow-2xl max-w-[816px] mx-auto min-h-[1056px] text-gray-900 font-serif print:shadow-none print:p-0 ring-1 ring-gray-200 print:ring-0">
        ...
        <p className="text-center text-sm mb-12 italic">(for COLB)</p>
