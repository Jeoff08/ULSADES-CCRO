...
import {
  ...
  deleteSavedWronglyRegister,
  getSavedWronglyRegisterList,
  loadSavedWronglyRegisterToDraft,
...
export default function WronglyRegisterSaved() {
  const navigate = useNavigate()
  const [rev, setRev] = useState(0)
  const [uploadsRev, setUploadsRev] = useState(0)
  const [searchQuery, setSearchQuery] = useState('')
  const list = useMemo(() => getSavedWronglyRegisterList().sort((a, b) => new Date(b.savedAt) - new Date(a.savedAt)), [rev, uploadsRev])
  const filtered = useMemo(() => {
    const q = searchQuery.trim().toLowerCase()
    if (!q) return list
    return list.filter((item) => {
      const label = String(item.label || 'Wrongly Register').toLowerCase()
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        {list.length > 0 ? (
          <div className="flex flex-wrap items-center justify-end gap-2 ml-auto">
            ...
            <button type="button" onClick={() => setSearchQuery('')} className="px-3 py-2.5 border border-gray-300 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition-all active:scale-95">
              Clear
            </button>
          </div>
        ) : null}
          ...
          {filtered.map((item, idx) => (
            ...
            >
              <div className="min-w-0">
                <p className="font-medium text-gray-800 truncate">{item.label || 'Wrongly Register'}</p>
                <p className="text-xs text-gray-500 mt-0.5">Wrongly Register · {formatSavedAt(item.savedAt)}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {hasAnyUploadsForRecord('wrongly-register', item.id) ? (
                  <span className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-amber-50/90 border border-amber-200/90" title="Has uploaded file(s)">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (loadSavedWronglyRegisterToDraft(item.id)) navigate('/legal-instrument/wrongly-register')
                  }}
                  className="px-3 py-1.5 border border-[var(--primary-blue)] text-[var(--primary-blue)] text-sm font-medium rounded-lg hover:bg-[var(--primary-blue)]/10 transition-all active:scale-95"
                >
                  Edit
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (loadSavedWronglyRegisterToDraft(item.id)) navigate('/legal-instrument/wrongly-register/print')
                  }}
                  className="px-3 py-1.5 bg-[var(--primary-blue)] text-white text-sm font-medium rounded-lg hover:bg-[var(--primary-blue-light)] transition-all active:scale-95"
                >
                  View &amp; Print
                </button>
                <button
                  type="button"
                  onClick={() => {
                    deleteSavedWronglyRegister(item.id)
                    setRev((v) => v + 1)
                  }}
                  className="px-3 py-1.5 border border-gray-300 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 hover:border-red-200 hover:text-red-600 transition-all active:scale-95"
                >
                  Remove
                </button>
              </div>
