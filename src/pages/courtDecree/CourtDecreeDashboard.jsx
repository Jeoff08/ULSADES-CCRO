...
export default function CourtDecreeDashboard() {
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-base font-bold text-gray-800 mb-1">Court Decree Automated Data Entry</h1>
        <p className="text-sm text-gray-500">Choose a document type to start data entry.</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {COURT_DECREE_TYPES.map((doc) => (
          <Link
            key={doc.id}
            to={{ pathname: '/court-decree/form', search: `?type=${doc.id}` }}
