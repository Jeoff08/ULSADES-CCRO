...
/**
 * ANNOTATION FOR FORM 1A – court decree.
 * When a Certificate of Live Birth (Form 102) is attached, the REMARKS/ANNOTATIONS area
...
export default function AnnotationForForm1A({ paperSize = 'a4', data, onAttachmentChange }) {
  ...
  useEffect(() => {
    if (!data?.annotationForm1AScanDataUrl) {
      setDisplayImageUrl(null)
      return
    }
    if (data.annotationForm1AScanDataUrl.startsWith('data:image/')) {
      setDisplayImageUrl(data.annotationForm1AScanDataUrl)
      return
    }
    if (data.annotationForm1AScanDataUrl.startsWith('data:application/pdf')) {
      ensureImageDataUrl(data.annotationForm1AScanDataUrl).then(setDisplayImageUrl).catch(() => setDisplayImageUrl(null))
      return
    }
    setDisplayImageUrl(null)
  ...
  const wrapperClass = [
    ...
    'colb-annotation-form-1a',
    'court-decree-annotation-form-1a',
    hasScan ? 'annotation-form-1a-print-attachment-only' : '',
  ].filter(Boolean).join(' ')
  ...
  return (
    <div className={wrapperClass} data-paper-size={paperSize}>
      ...
      <div className="print-doc-body flex flex-col flex-1 min-h-0">
        ...
        <div className="no-print mb-4 flex flex-wrap items-center gap-3">
          ...
          {hasScan && displayImageUrl && (
            <button
              type="button"
              onClick={() => exportColbAsPdf(colbExportRef.current, `Annotation-Form1A-${Date.now()}.pdf`)}
        ...
        {hasScan && displayImageUrl ? (
          <div className="colb-print-area relative mb-4">
            <p className="text-xs font-medium text-gray-600 mb-1 print:hidden">Attached Form 102 (Certificate of Live Birth)</p>
            <div
              ref={colbExportRef}
              className="relative w-full max-h-[600px] print:max-h-[275mm] mx-auto overflow-visible colb-certificate-container colb-form1a-full-height"
              style={{
            ...
            >
              ...
              <img
                src={displayImageUrl}
                alt="Form 102 Certificate of Live Birth"
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 print:hidden" role="dialog" aria-modal="true" aria-labelledby="upload-modal-title">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between px-6 pt-6 pb-2">
              <div>
                <h3 id="upload-modal-title" className="text-lg font-bold text-gray-900">Upload File</h3>
